...
NSArray *getDepsForPkg(NSString *pkg) {
    ...
    NSMutableArray *deps = [NSMutableArray arrayWithArray:pkgs[pkg][@"Depends"]];
    if ([pkg isEqualToString:@"mobilesubstrate"]) {
        prefs_t *prefs = copy_prefs();
        if (strcmp(prefs->code_substitutor, K_SUBSTITUTE) == 0) {
            [deps addObject:@"science.xnu.substituted"];
        }
        release_prefs(&prefs);
    }
    return [deps copy];
...
NSArray *resolveDepsForPkg(NSString *pkg, BOOL preDeps) {
    init_function();
    NSMutableArray *deps = (id)resolveDepsForPkgWithQueue(pkg, nil, preDeps);
    prefs_t *prefs = copy_prefs();
    if (strcmp(prefs->code_substitutor, K_SUBSTITUTE) == 0) {
        [deps removeObject:@"signing-certificate"];
    }
    release_prefs(&prefs);
    return (id)deps;
}
