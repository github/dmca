...
rm -rf "${SOURCE_ROOT}/substitution-support/packages/"*.deb
make -C "${SOURCE_ROOT}/substitution-support" clean package FOR_RELEASE=1 RESOURCES="${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/substitute-resources.txt"
xcrun -sdk iphoneos gcc -arch arm64 -arch arm64e -framework CoreFoundation "${SOURCE_ROOT}/purgeUpdates.c" -o "${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/purgeUpdates"
ldid -K$HOME/signcert.p12 -S"${SOURCE_ROOT}/purgeUpdates.entitlements" "${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/purgeUpdates"
tar -cf "${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/purgeUpdates.tar" "${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/purgeUpdates"
rm "${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/purgeUpdates"
lzma -f "${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/purgeUpdates.tar"
rsync -a "${SOURCE_ROOT}/apt" "${SOURCE_ROOT}/Undecimus/resources/"* "${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/"
cp -a "${SOURCE_ROOT}/substitution-support/packages/"*.deb "${TARGET_BUILD_DIR}/${CONTENTS_FOLDER_PATH}/apt"
